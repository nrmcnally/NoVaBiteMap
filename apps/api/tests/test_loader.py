...
def test_seed_counts_match_audited_dataset(seeded_db):
    ...
    with SessionLocal() as s:
        assert s.scalar(select(func.count()).select_from(FishingLocationRecord)) == 67
        assert s.scalar(select(func.count()).select_from(SpeciesRecord)) == 20
        assert s.scalar(select(func.count()).select_from(SpeciesEvidenceRecord)) == 99
        ...
        assert s.scalar(select(func.count(func.distinct(SpeciesEvidenceRecord.location_id)))) == 45
